...
const (
	...
	UNLOCK_FLAG_SUCCEED_TO_LOCK_WAIT            = 0x08
...
const (
	...
	TIMEOUT_FLAG_LESS_LOCK_VERSION_IS_LOCK_SUCCEED     = 0x4000
...
func NewStateCommand(dbId uint8) *StateCommand {
	command := Command{Magic: MAGIC, Version: VERSION, CommandType: CommandState, RequestId: GenRequestId()}
...
func (self *StateResultCommand) Decode(buf []byte) error {
	...
	self.State.ExpiredCount = uint32(buf[51]) | uint32(buf[52])<<8 | uint32(buf[53])<<16 | uint32(buf[54])<<24
...
func (self *StateResultCommand) Encode(buf []byte) error {
	...
	buf[51] = byte(self.State.ExpiredCount)
	buf[52] = byte(self.State.ExpiredCount >> 8)
	buf[53] = byte(self.State.ExpiredCount >> 16)
	buf[54] = byte(self.State.ExpiredCount >> 24)
