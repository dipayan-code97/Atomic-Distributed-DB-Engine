...
func (selfRef *TextRequestCommand) Decode(buf []byte) error {
	if len(buf) > len(selfRef.Parser.rbuf) || selfRef.Parser.bufIndex != selfRef.Parser.bufLen {
		return errors.New("buf error")
	}

	copy(selfRef.Parser.rbuf, buf)
	selfRef.Parser.bufLen = len(buf)
	selfRef.Parser.bufIndex = 0
	return selfRef.Parser.ParseRequest()
}

func (selfRef *TextRequestCommand) Encode(buf []byte) error {
	buildBuf := selfRef.Parser.BuildRequest(selfRef.Args)
	if len(buildBuf) > len(buf) || selfRef.Parser.bufIndex == selfRef.Parser.bufLen {
		return errors.New("buf error")
	}
...
func (selfRef *TextRequestCommand) GetArgs() []string {
	return selfRef.Args
}
