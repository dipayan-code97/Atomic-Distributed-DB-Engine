...
func NewSemaphore(db *Database, semaphoreKey [16]byte, timeout time.Duration, expried uint32, count uint16) *Semaphore {
	if count > 0 {
		count = count - 1
	}
...
func (self *Semaphore) Acquire() *LockError {
	lock := &Lock{self.db, self.db.generateLockID(), self.semaphoreKey, self.timeout, self.expried, self.count, 0}
	return lock.lock()
}
...
func (self *Semaphore) Count() (int, *LockError) {
	lock := &Lock{self.db, self.db.generateLockID(), self.semaphoreKey, 0, 0, self.count, 0}
	err := lock.LockShow()
