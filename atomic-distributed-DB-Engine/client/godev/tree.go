...
func (self *TreeLock) NewLeafLock() *TreeLeafLock {
	lock := &Lock{self.db, self.db.generateLockID(), self.lockKey, self.timeout, self.expried, 0xffff, 0}
...
func (self *TreeLock) NewChild() *TreeLock {
	return &TreeLock{self.db, self.lockKey, self.db.generateLockID(), self.timeout, self.expried, false, nil}
}
...
func (self *TreeLock) Lock() *LockError {
	checkLock := &Lock{self.db, self.db.generateLockID(), self.lockKey, self.timeout, 0, 0, 0}
	err := checkLock.lock()
	if err != nil {
		return err
	}
...
func (self *TreeLock) Wait(timeout uint32) *LockError {
	checkLock := &Lock{self.db, self.db.generateLockID(), self.lockKey, timeout, 0, 0, 0}
