...
func (self *SLock) GetState(serverProtocol ServerProtocol, command *protocol.StateCommand) error {
	...
	if db == nil {
		return serverProtocol.Write(protocol.NewStateResultCommand(command, protocol.RESULT_SUCCEED, 0, dbState, nil))
	}
	return serverProtocol.Write(protocol.NewStateResultCommand(command, protocol.RESULT_SUCCEED, 0, dbState, db.GetState()))
}
