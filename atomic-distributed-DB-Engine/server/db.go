...
func (self *LockDB) RemoveExpried(lock *Lock) {
	lock.expried = true
	lock.manager.state.ExpiredCount--
}
...
func (self *LockDB) doExpried(lock *Lock, forcedExpried bool) {
	...
	lockManager.state.ExpiredCount++
	lockManager.glock.Unlock()
	...
	*/
	...
	if lockManager.locked > 0 {
		...
		if currentLock != nil {
			...
			if command.Flag&protocol.LOCK_FLAG_UPDATE_WHEN_LOCKED != 0 {
				...
				_ = serverProtocol.ProcessLockResultCommand(command, protocol.RESULT_SUCCEED, uint16(lockManager.locked), currentLock.locked)
				_ = serverProtocol.FreeLockCommand(command)
				return nil
			} else {
				lockManager.glock.Unlock()
	...
	if !waited && self.doLock(lockManager, lock) {
		...
		if command.Expried > 0 {
			...
			_ = serverProtocol.ProcessLockResultCommand(command, protocol.RESULT_SUCCEED, uint16(lockManager.locked), lock.locked)
	...
	if command.TimeoutFlag&protocol.TIMEOUT_FLAG_LESS_LOCK_VERSION_IS_LOCK_SUCCEED != 0 {
...
func (self *LockDB) UnLock(serverProtocol ServerProtocol, command *protocol.LockCommand) error {
	/*
	   ...
	   |7                  |        4       |         3         |    2   |           1             |               0               |
	   |-------------------|----------------|-------------------|--------|-------------------------|-------------------------------|
	   |                   |unlock_tree_lock|succed_to_lock_wait|from_aof|when_unlocked_cancel_wait|when_unlocked_unlock_first_lock|
	*/
	...
	if currentLock.locked > 1 {
		...
		if command.Flag&protocol.UNLOCK_FLAG_SUCCEED_TO_LOCK_WAIT != 0 {
...
func (self *LockDB) wakeUpWaitLock(lockManager *LockManager, waitLock *Lock, serverProtocol ServerProtocol) {
	...
	if waitLock.command.Expried > 0 {
		...
		if waitLockProtocol.serverProtocol == serverProtocol {
			_ = serverProtocol.ProcessLockResultCommand(waitLockCommand, protocol.RESULT_SUCCEED, uint16(lockManager.locked), waitLock.locked)
		} else {
			_ = waitLockProtocol.ProcessLockResultCommandLocked(waitLockCommand, protocol.RESULT_SUCCEED, uint16(lockManager.locked), waitLock.locked)
		}
		return
	}
...
func (self *LockDB) DoAckLock(lock *Lock, succed bool) {
	...
	if succed {
		...
		_ = lockProtocol.ProcessLockResultCommandLocked(lockCommand, protocol.RESULT_SUCCEED, uint16(lockManager.locked), lock.locked)
		return
	}
...
func (self *LockDB) GetState() *protocol.LockDBState {
	...
	for _, s := range self.states {
		...
		state.ExpiredCount += s.ExpiredCount
